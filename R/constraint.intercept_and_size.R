#'@title Constraint on trait variance when keeping intercept and size fixed.
#' 
...
#' 
#'@return Constraint on trait variance when keeping intercept and size fixed.
#'
...
{
  out<-(var(allometry.object$x_mean)) * ((var(allometry.object$intercept)*var(allometry.object$slope)*var(allometry.object$x_mean)) + 
                                           ...
                                           (cov(allometry.object$slope,allometry.object$x_mean)^2)*var(allometry.object$intercept))/((var(allometry.object$intercept)*var(allometry.object$x_mean))-cov(allometry.object$intercept,allometry.object$x_mean)^2)
  return(out)
}